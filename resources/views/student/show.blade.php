@extends('dashboard.index')

@section('title')
    Información de {{ $student->name }} {{ $student->last_name }}
@endsection

@section('dash-content')
    <div>
        <h4 class="fw-bold py-3 mb-4">
            <span class="text-muted fw-light">
                <a href="{{ route('index') }}">Inicio</a> /
                <a href="{{ route('students.index') }}">Lista de estudiantes</a> /
            </span>
            Información del estudiante
        </h4>
        <div class="container">
            <div class="row">
                <div class="col-lg-4">
                    <div class="card mb-4">
                        <div class="card-body text-center">
                            <img src="{{ asset('assets/img/avatars/student.jpg') }}" alt="avatar"
                                class="rounded-circle img-fluid" style="width: 150px;">
                            <h5 class="my-3">{{ $student->name }} {{ $student->last_name }}</h5>
                        </div>
                    </div>
                </div>
                <div class="col-lg-8">
                    <div class="card mb-4">
                        <div class="card-body">
                            <div class="row">
                                <div class="col-sm-3">
                                    <p class="mb-0">Nombre Completo</p>
                                </div>
                                <div class="col-sm-9">
                                    <p class="text-muted mb-0">{{ $student->name }} {{ $student->last_name }}</p>
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-sm-3">
                                    <p class="mb-0">Aula</p>
                                </div>
                                <div class="col-sm-9">
                                    <p class="text-muted mb-0">
                                        @if (isset($student->classroom_id))
                                            {{ $student->getClassroom->code }} | {{ $student->getClassroom->course }}
                                        @else
                                            Aula no asignada
                                        @endif
                                    </p>
                                </div>
                            </div>
                            <hr>
                            <div class="row">
                                <div class="col-sm-3">
                                    <p class="mb-0">Edad</p>
                                </div>
                                <div class="col-sm-9">
                                    <p class="text-muted mb-0">{{ $student->age }} Años</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="card mb-4 mb-md-0">
                                <div class="card-body">
                                    <p class="mb-4"><span class="text-primary font-italic me-1">Información
                                            adicional</span></p>
                                    <p class="mb-1">Fecha de registro:
                                        {{ date('d-m-Y', strtotime($student->created_at)) }}</p>
                                    <p class="mb-1">Ultima actualización:
                                        {{ date('d-m-Y', strtotime($student->updated_at)) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection
